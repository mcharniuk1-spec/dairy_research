...
PROFILE_KEYWORDS = {
    "Яйця курячі": {"яйце", "яйця", "яєч", "яй", "куряч", "десяток", "egg", "eggs"},
    ...
    "Йогурт": {"йогурт", "йогур", "біфідойогурт", "біфойогурт"},
    "Сир твердий/напівтвердий": {"сир", "тверд", "напівтверд", "гауда", "моцарел", "чеддер", "пармезан"},
    "Сир кисломолочний": {"сир", "кисломол", "творог", "творож", "зернист", "кислом"},
    "Сметана": {"сметан"},
...
def assign_profile(
    ...
    if re.search(r"\b(?:яй\w*|egg|eggs)\b", title):
        return "Яйця курячі"
    ...
    if re.search(r"\bсметан", title):
        return "Сметана"
    if re.search(r"\bсир\s+кислом|\bкисломол|\bтворог|\bтворож", title):
        return "Сир кисломолочний"
    if re.search(r"\bйогурт|\bйогур|\bбіфідойогурт|\bбіфойогурт", title):
        return "Йогурт"
    # Масло вершкове: якщо в назві є "масло".
    if re.search(r"\bмасл", title):
        return "Масло вершкове"
    # Сир твердий/напівтвердий: будь-який "сир", який НЕ кисломолочний.
    if re.search(r"\bсир\b", title) and not re.search(r"\bсир\s+кислом|\bкисломол|\bтворог|\bтворож", title):
        return "Сир твердий/напівтвердий"
